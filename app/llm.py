...
- 挨拶・雑談は intent=chat。answer は120字以内。
- 相談（「あと何食べていい」「何を食べたらいい」「足りてる？」等）は intent=question。answer には以下を含める:
  1. 残り摂取可能kcalと、不足している栄養素（タンパク質など）の具体的な量
  2. 残り予算に収まる具体的な献立を2〜3品。各品に推定kcalを付ける
     （例: ・鶏むねのグリル(150g) 約250kcal / ・納豆ごはん 約350kcal）
  3. タンパク質不足が大きい日は、高タンパクの選択肢を優先する
  4. 文量は400字以内。箇条書き可。
